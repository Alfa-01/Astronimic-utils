...
def main() -> None:
    ...
    if user_choice == "1":
        latitudes, longitudes = calc.SystemConverter.coordinates_for_sphere()
        plot_builder: draw.Plot = draw.SphericalPlot(10, 8)
        plot_builder.show_sphere(consts.EARTH_RADIUS)
        plot_builder.show_trajectory(longitudes, latitudes, consts.EARTH_RADIUS, "red")
    elif user_choice == "2":
        latitudes, longitudes = calc.SystemConverter.coordinates_for_map()
        plot_builder: draw.Plot = draw.MapPlot(10, 8)
        plot_builder.show_map()
        plot_builder.show_trajectory(longitudes, latitudes, "red")
    elif user_choice == "3":
        plot_builder: draw.Plot = draw.EllipsoidsPlot(10, 8)
        plot_builder.show_ellipsoid(consts.a, consts.b)
        plot_builder.show_trajectory(
            "red", **dict(zip(("x", "y", "z"), calc.SystemConverter.coordinates_for_ellipsoid()))
        )
    else:
